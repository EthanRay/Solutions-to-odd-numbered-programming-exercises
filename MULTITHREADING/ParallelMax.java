	...
	private static class MaxTask extends RecursiveTask<Integer>{
		...
		protected Integer compute() {
			if(high-low<THRESHOLD){
				...
				int mid=(low+high)/2;
				RecursiveTask<Integer>lefTask=new MaxTask(list, low, mid);
				RecursiveTask<Integer>rightTask=new MaxTask(list, mid, high);
				rightTask.fork();
				lefTask.fork();
				return new Integer(Math.min(lefTask.join().intValue(), rightTask.join().intValue()));
			}
		}
